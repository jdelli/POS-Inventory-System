    ...
    <div className="fixed inset-0 flex items-center justify-center bg-gray-500 bg-opacity-75 z-50">
      <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-2xl h-auto flex flex-col relative receipt-content print:w-full">
        ...
        <div className="mb-4">
          ...
          <p><strong>Receipt Number:</strong> {selectedOrder.receipt_number}</p>
          ...
          <tbody>
            {selectedOrder.items.map((item, index) => (
              <tr key={index} className={index % 2 === 0 ? 'bg-gray-50' : ''}>
                ...
                <td className="border p-2">₱{Number(item.price.toLocaleString() || 0).toLocaleString()}</td>
                <td className="border p-2">{item.quantity}</td>
                <td className="border p-2">₱{(Number(item.price.toLocaleString() || 0) * Number(item.quantity || 0)).toLocaleString()}</td>
              </tr>
            ))}
            {/* Grand Total Row */}
            <tr>
        ...
        <div className="mt-4 border-t pt-2 text-center">
          ...
          {/* <p className="text-sm">Visit us at www.companywebsite.com</p> */}
        </div>

        <div className="flex justify-end mt-4 no-print">
          <button onClick={onClose} className="bg-blue-500 text-white p-2 rounded mr-4">Close</button>
